...
def test_search(capsys):
    ...
    assert dl2.search("model", limit=1, info=True)[1]["index"] == "dlhub-test"
...
def test_match_field():
    ...
    res2 = dl.search()
    # NOTE change to assertion == 1 after more (physics) models ingested to search
...
def test_match_range():
    ...
    res6 = dl.match_range(field="datacite.publicationYear", start=2020, stop=None, inclusive=True).search()
    assert res6 != []
